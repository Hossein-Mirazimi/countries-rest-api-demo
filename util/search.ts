...
export const checkIfStringsContainSameCharacters = (string1: string, string2: string) => {
...
export const findBestMatch = (mainString: string, targetStrings: {name: string}[]) => {
  const ratings = [];
...
export const sortCallback = (a: any, b: any, key: string, secondKey?: string) => {
  if (a[key] > b[key]) {
    return -1;
  }
  else if (a[key] < b[key]) {
